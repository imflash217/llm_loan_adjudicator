...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(
            ...
            Path(
                cfg.CHECKPOINTS_DIR_PATH,
                f"ckpt_{cfg.BASE_MODEL_NAME}_epoch_{epoch}.pth",
            ),
        )
    ...
    torch.save(model.state_dict(), cfg.FINETUNED_MODEL_FPATH)
    print(f"Finetuned model saved as: [{cfg.FINETUNED_MODEL_FPATH}]")
...
def calc_loss_loader(data_loader, model, device, num_batches=None):
    ...
    if len(data_loader) == 0:
        ...
        # Reducing the number of batches to match the total number of batches in the dataloader
...
def plot_losses(epochs_seen, tokens_seen, train_losses, val_losses):
    ...
    fig.tight_layout()
    plt.savefig(cfg.LOSS_PLOT_FIG_FPATH)
